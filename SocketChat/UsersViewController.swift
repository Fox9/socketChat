...
class UsersViewController: UIViewController, UITableViewDelegate, UITableViewDataSource {
    ...
    func askForNickname() {
        let alerController = UIAlertController(title: "SocketChat", message: "Please enter a name", preferredStyle: .alert)
        alerController.addTextField(configurationHandler: nil)
        
        let oKAction = UIAlertAction(title: "OK", style: .default) { (action) in
            let textfield = alerController.textFields![0]
    ...
    @IBAction func joinToChatButton(_ sender: UIButton) {
...
//                let chatViewController = segue.destination as! ChatLogController
